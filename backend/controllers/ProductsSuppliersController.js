...
const ProductsSuppliersController = {
  link(req, res) {
    try {
      ...
      const sql = `INSERT INTO products_suppliers (product_id, supplier_id) VALUES (?, ?)`;
      const result = db.prepare(sql).run(product_id, supplier_id);
      res.status(201).json({ message: "Link created successfully", id: result.lastInsertRowid });
    } catch (err) {
  ...
  unlink(req, res) {
    try {
      ...
      const sql = `DELETE FROM products_suppliers WHERE product_id = ? AND supplier_id = ?`;
      const result = db.prepare(sql).run(product_id, supplier_id);
      if (result.changes === 0) {
  ...
  suppliersByProduct(req, res) {
    try {
      ...
      const sql = `
        ...
        WHERE ps.product_id = ?
      `;
      const rows = db.prepare(sql).all(product_id);
      res.json(rows);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  productsBySupplier(req, res) {
    try {
      ...
      `;
      const rows = db.prepare(sql).all(supplier_id);
      res.json(rows);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
